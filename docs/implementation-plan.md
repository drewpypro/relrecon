...
├── data/                     # Input datasets (CSV, TSV or Parquet)
...
- Resolve dataset paths (CSV, TSV and Parquet -- auto-detected from file extension)
...
| 3 | Multi-match resolution | Configurable per recipe: `best_match` (default -- prefer core_parent, then highest score) or `all_matches` (skip dedup -- returns all candidates from all steps as-is, useful for analysis/debugging). |
...
| 5 | Input formats | Support CSV, TSV and Parquet. Configured per source in recipe (auto-detected from file extension). |
